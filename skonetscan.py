...
def resolve_hostname(ip: str) -> str:
    ...
    # -----------------------------
    # 2. mDNS via avahi-resolve-address
    # -----------------------------
    try:
        p = subprocess.run(["avahi-resolve-address", ip],
                           capture_output=True, text=True, timeout=3)
        m = re.search(r"=\s+(.+)$", p.stdout)
        if m:
    ...
    # -----------------------------
    # 3. NetBIOS via nmblookup (Windows hosts)
    # -----------------------------
    try:
        p = subprocess.run(["nmblookup", "-A", ip],
                           capture_output=True, text=True, timeout=3)
    ...
    # -----------------------------
    # 4. Nmap reverse lookup
    # -----------------------------
    try:
        p = subprocess.run(["nmap", "-sL", ip],
                           capture_output=True, text=True, timeout=4)
        m = re.search(r"Hostname:\s+([^\s]+)", p.stdout)
    ...
    # -----------------------------
    # 5. MAC vendor lookup for fallback name
    # -----------------------------
    try:
        mac = get_mac(ip)
        if mac:
...
def lookup_mac_vendor(mac: str) -> str:
    ...
    oui = mac.upper()[0:8]
    return prefixes.get(oui, "UnknownVendor")
